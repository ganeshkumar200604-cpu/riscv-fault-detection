...
void inject_memory_fault() {
    // Simulate memory fault by doing excessive
    // unaligned and repeated memory read/write operations
    // This creates abnormal memory access patterns
    volatile uint32_t buf[64];
    for (uint32_t i = 0; i < 64; i++) buf[i] = i * 0xDEAD;
    for (uint32_t i = 63; i > 0; i--) buf[i-1] ^= buf[i];
    for (uint32_t i = 0; i < 64; i++) buf[i] += buf[(i+1)%64];
    volatile uint32_t dummy = buf[0];
    (void)dummy;
}
